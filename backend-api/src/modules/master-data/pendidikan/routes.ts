...
import { PendidikanCreate, PendidikanListQuery, PendidikanUpdate } from "./schema";
import {
  createData,
  deleteData,
  dataById,
...
export const pendidikanRoutes = new Elysia({ prefix: "/pendidikan" })
  .get(
    "/",
    async ({ query }) => {
  ...
  .post(
    ...
    { beforeHandle: [requireRole(["admin", "operator"])], body: PendidikanCreate }
  )
  .put(
    "/:id",
    ...
    { beforeHandle: [requireRole(["admin", "operator"])], body: PendidikanUpdate }
  )
  .delete(
    "/:id",
    async ({ params, set }) => {
