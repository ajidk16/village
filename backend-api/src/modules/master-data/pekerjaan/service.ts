...
import { pekerjaan } from "@/db/schema";
...
export async function listData(query: {
  ...
  const searchCols = [pekerjaan.name, pekerjaan.status];

  const filters: (SQL<unknown> | undefined)[] = [
    query.name ? ilike(pekerjaan.name, `%${query.name}%`) : undefined,
    query.status ? ilike(pekerjaan.status, `%${query.status}%`) : undefined,
  ];
  ...
  return {
    status: 200,
    message: "successfully retrieved pekerjaan",
    items: rows,
    page,
    limit,
...
export async function dataById(id: number) {
  const [row] = await db.select().from(pekerjaan).limit(1);
...
export async function createData(input: { name: string; status: boolean }) {
  try {
    const [created] = await db.insert(pekerjaan).values(input);
...
export async function deleteData(id: number) {
  const [deleted] = await db.delete(pekerjaan).where(eq(pekerjaan.id, id)).returning();
